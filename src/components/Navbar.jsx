    ...
    <div className="w-[85px] sm:w-[68px] h-full fixed left-0 top-0 flex flex-col justify-between border-r borde-gray-200 px-4 py-10 xl:py-6 z-10">
        ...
        <div className="flex flex-col gap-y-3 sm:gap-y-2 xl:gap-y-1 xs:gap-y-0">
            {navbarData.map((item, i) => (
                ...
                <span className={`text-[10px] sm:text-[8px] tracking-wider -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 text-center  dark:text-white ${i % 2 === 0 ? '-translate-x-2' : '-translate-x-2'} ${item.id === id && '-translate-x-0 opacity-100'}`}>{item.name}</span>
            </a>
            ))}
        </div>
        <p className="flex items-center justify-center text-[13px] text-gray-500 mt-6 xs:text-[11px]">
            <span className="absolute left-1/2 w-max flex items-center -rotate-90 origin-bottom-left tracking-wider dark:text-gray-200 transition-colors">{copyRightIcon} 2022 - {new Date().getFullYear()}</span>
        </p>
    </div>
  )
}
